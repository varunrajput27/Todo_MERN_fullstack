...
const Todoapp = () => {
    ...
    // Fetch all tasks on component mount (and whenever username changes)
    useEffect(() => {
        if (username) {
            axios
                .get(`${import.meta.env.VITE_BACKEND_URL}/field?username=${username}`)
                .then((response) => {
                    setTasks(response.data.tasks || []);
                })
                .catch((error) => {
    ...
    const handlesubmit = (e) => {
        if (data.trim() !== '') {
            axios
                .post(`${import.meta.env.VITE_BACKEND_URL}/field`, { username, tasks: [{ text: data, completed: false }] })
                .then(() => {
                    setTasks((prev) => [...prev, { text: data, completed: false }]);
                    setData('');
                })
    ...
    const taskComplete = (index) => {
        ...
        setTasks(newTasks); // Update frontend

        // Send the completion status to the backend
        axios
            .put(`${import.meta.env.VITE_BACKEND_URL}/field/update-task`, {
                username: username,
                taskText: task.text,
    ...
    const taskdelete = (index) => {
        const taskToDelete = tasks[index];
        axios
            .delete(`${import.meta.env.VITE_BACKEND_URL}/field`, {
                data: {
                    username: username,
                    taskText: taskToDelete.text,
                },
    ...
    // Handle enter key for submitting task
    const handlekeydown = (e) => {
        if (e.keyCode === 13) {
            e.preventDefault();
            handlesubmit();
    ...
    return (
        <div className="bg-zinc-950 h-screen flex justify-center items-start">
            <div className="w-full sm:w-96 h-[90vh] p-6 mt-3 shadow-lg rounded-2xl bg-gray-900/50 border border-gray-800 relative">
                ...
                {/* Logout button */}
                <div className="text-xl font-semibold text-center absolute bottom-4 left-5 right-5">
                    <button className="bg-purple-700 rounded-md text-white p-1 w-full cursor-pointer" onClick={exit}>
                        Logout
                    </button>
                </div>
            </div>
